...
col3.metric("Saldo Acumulado", f"R$ {saldo_acumulado:,.2f}")
...
fig1 = go.Figure()
fig1.add_trace(go.Bar(name='Entradas',
               x=df['Mês'], y=df['Entradas'], marker=dict(color='#19AAE1')))
fig1.add_trace(
    go.Bar(name='Saídas', x=df['Mês'], y=df['Saídas'], marker=dict(color='orange')))
fig1.update_layout(
